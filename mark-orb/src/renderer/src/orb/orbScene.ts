...
export interface OrbSceneApi {
  ...
  resetView(): void;
  dispose(): void;
  /** Drive the core's glow/surge from an external source (0 = idle, 1 = speaking/listening peak). */
  setEnergy(level: number): void;
  /** Pause/unpause 3D animation loop for zero CPU overhead. */
...
export function createOrbScene(container: HTMLElement): OrbSceneApi {
  ...
  const chromaticShader = {
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        gl_FragColor = vec4(cr.r, cg.g * 0.95, cb.b * 1.15, 1.0) * flicker;
        // Push towards cool electric blue tone
        gl_FragColor.rgb = mix(gl_FragColor.rgb, gl_FragColor.rgb * vec3(0.6, 0.85, 1.25), 0.35);
      }
    `,
  };
  ...
  // ——— COLORS (Deep Electric Blue theme) ———
  const C_BRIGHT = 0x4fc3f7;  // light sky blue
  const C_MID   = 0x0288d1;  // deep ocean blue
  const C_DIM   = 0x01579b;  // dark navy
  const C_FAINT = 0x0d2137;  // very dark blue
  const C_HOT   = 0x80d8ff;  // ice/white-blue
  ...
  function makeTextSprite(text: string, size = 0.08) {
    ...
    ctx.fillStyle = `rgba(${(20 + Math.random() * 40) | 0}, ${(160 + Math.random() * 60) | 0}, 255, ${alpha})`;
  ...
  g.addColorStop(0,   "rgba(100,210,255,1)");
  g.addColorStop(0.2, "rgba(30,140,255,0.6)");
  ...
  // Voice-reactive energy (0..1), smoothed toward its target each frame in animate().
  let energyTarget = 0;
  let energy = 0;
  function setEnergy(level: number) {
    energyTarget = THREE.MathUtils.clamp(level, 0, 1);
  }
  ...
  function animate() {
    ...
    energy += (energyTarget - energy) * 0.08; // smooth toward target
    const surge = wave3 * 1.5 + wave4 * 2.0 + energy * 1.3;
  ...
  function setLiveListenMode(active: boolean) {
    if (active) {
      // Live Listen: Electric Cyan / Quantum Green theme
      dustMat.color.setHex(0x00e5ff);
      coreSphereMat.color.setHex(0x00bcd4);
      glowSphereMat.color.setHex(0x00e676);
      icoWireMat.color.setHex(0xffffff);
      chromaticShader.uniforms.uIntensity.value = 0.008;
    } else {
      // PTT: restore Deep Electric Blue defaults
      ...
      chromaticShader.uniforms.uIntensity.value = 0.003;
    }
  }
